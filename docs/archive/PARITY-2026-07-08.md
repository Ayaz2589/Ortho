# Ortho cross-surface parity (ARCHIVED — pre-021 snapshot, 2026-07-08)

> **This is a frozen historical snapshot**, verbatim as of just before spec 021
> (Capacitor iOS consolidation) retired the native SwiftUI app. It describes a
> "one product on three surfaces, two live implementations" model that is no
> longer current. For the live, maintained parity contract see the root
> [`PARITY.md`](../../PARITY.md). Kept here as an audit trail, not to be edited.
...
full report in `docs/parity-audit-2026-07-02.md` (removed 2026-07-19 — retrieve from git history if
needed). All 65 were fixed the

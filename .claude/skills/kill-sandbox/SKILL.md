---
name: kill-sandbox
description: Use when killing/removing/tearing down a Docker Sandbox (`sbx`) for the Ortho repo identified by its git branch — e.g. "/kill-sandbox feat/savings-goals" — or when the sandbox registry needs reconciling. Resolves the branch to its sandbox, removes it after a safety check for unpushed work, and keeps docs/sandbox/sandbox-history.md (active + killed history) current. Host-only — `sbx` is not available inside a sandbox.
user-invocable: true
...
Remove a Docker Sandbox for the Ortho repo **by its git branch** (not its `sbx`
name), safely, and keep the project's sandbox registry
(`docs/sandbox/sandbox-history.md`) up to date.
...
Companion to the **docker-sandbox** skill (creation/management). Read its
"Environment guard" and "Gotchas" — they apply here verbatim.
...
- Run `sbx ls`. Keep only sandboxes whose **workspace is the Ortho repo path**
  (ignore other projects).
- Read `docs/sandbox/sandbox-history.md` (create it from the template below if
  missing). Diff `sbx ls` against the **Active** table:
...
- **Miss?** Probe live sandboxes:
  ...
  Ortho sandbox. **Warn the user first: probing starts stopped sandboxes.**
...
Host-maintained by this skill + docker-sandbox. Not hand-edited; not edited from
inside a sandbox (state is host-local). If missing, create it with this shape:
...
## Sources
- Repo `.claude/skills/docker-sandbox/SKILL.md` — environment guard, gotchas

...
| **III. Right Form Factor Per Canvas** | ✅ Whole-UI proportional scale preserves every canvas's layout; media-query breakpoints are unaffected by standardized `zoom`, so the phone/desktop layout choice never flips unexpectedly. Safe-area insets and the fixed tab bar stay correct (see research.md R2). |

...
| [./environments.md](./environments.md) | …touching the **deploy pipeline or environments**: the local/stage/prod model + the `appEnv()` discriminator, the `main`→prod migrate-before-deploy pipeline (`web-deploy.yml` + `vercel.json` disabling Vercel's main auto-deploy), the `staging` branch → Vercel custom env + `supabase-migrations-staging.yml`, both Supabase projects (prod `brujhxmtzfgowimprueo` / stage `oozwqzsfbtkzywsxrzdq`), the GH Actions secrets/vars matrix, auto-login, and the "Sensitive vars break the build" gotcha. |

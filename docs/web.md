...
- **Vercel** (project `ortho`, team `ayaz2589s-projects`): **`main`→prod is NOT Vercel Git
  auto-deploy** — `web/vercel.json` disables it (`deploymentEnabled.main:false`); prod ships via
  `.github/workflows/web-deploy.yml` (migrate-then-deploy over the Vercel CLI). The `staging` branch
  → the custom **`staging`** environment; other branches/PRs → preview URLs. Load-bearing settings:
  **Root Directory = `web`** (the #1 first-deploy failure) and the per-environment `NEXT_PUBLIC_*`
  vars (Vercel builds remotely — `.env.local` is not involved; env-less builds succeed on
  placeholders but can't sign in). **Full environment/deploy model: [./environments.md](./environments.md).**

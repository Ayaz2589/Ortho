...
export function SubscriptionSection() {
  ...
  const status = (() => {
    ...
    switch (gateState) {
      case 'trialing':
        return {
          line: t('Free month — {0} days left', daysRemaining(entitlement.access_expires_at, new Date().toISOString())),
    ...
    <section className="flex flex-col gap-2" aria-labelledby="subscription-label">
      ...
      <SectionCard>
        <div className="flex min-h-[60px] items-center gap-3 px-4 py-3">
          ...
          <span
            className="min-w-0 text-[15px] text-text"
            aria-live={gateState === 'grace' ? 'polite' : undefined}
          >
            {status.line}
          </span>

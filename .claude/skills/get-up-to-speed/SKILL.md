...
description: Recover working context after a context clear (or at the start of a fresh session). Reads the documentation the user points at — or, with no arguments, the standard recovery set (latest context summary, CLAUDE.md, README.md, active spec/plan/tasks docs) plus git state — and reports back what has been done so far and what the next steps are. Companion to /remember, which writes the summaries this skill reads.
user-invocable: true
...
2. `CLAUDE.md` at the repo root — architecture and project guidance. If it
   points at other docs (e.g. a plan or a docs index), follow those pointers.
3. `README.md` at the repo root — quick-start / operational knowledge.
4. **Active feature docs**: if the current git branch matches a spec directory
   (e.g. branch `014-receipt-statement-scan` → `specs/014-receipt-statement-scan/`),
   read its `spec.md`, `plan.md`, and `tasks.md` if present. `tasks.md` is the
   best source for done-vs-pending — note which tasks are checked off.
...
- If the project has CI-driven feedback (see CLAUDE.md), check the latest run
  status if it's cheap to do so (e.g. `gh run list --limit 3`).
...
overall state (e.g. "feature 014 is implemented through T036; CI is green;
T037 is operator-pending").
...
`/remember` and this skill are two halves of one loop:
...
it just leans harder on CLAUDE.md, README.md, feature docs, and git history,
and should mention that no session handoff was found.

## Adapt to the project at hand

This skill is project-agnostic. Adapt what "current state" means to the
project (deployment state for web apps, CI status for library work, positions
for trading systems, etc.). If the project's CLAUDE.md documents a
session-continuity convention, follow that convention first.

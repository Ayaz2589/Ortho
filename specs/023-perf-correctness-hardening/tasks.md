...
- [X] T003 [P] [US1] Write FAILING repro test in `web/test/web/txform-fx-split.test.tsx` (jsdom): GBP display currency, a 2-owner value split (dossier cases 2¢→1/1 and 11¢→2/9); assert the current code either false-blocks Save (`canSave===false`) or writes `sum(shares) !== amount_cents` on a no-op save. ✅ Repro confirmed failing (`shares` sum 12 ≠ amount_cents 11).
- [X] T004 [US1] Fix B1 in `web/components/web/TxForm.tsx`: on a no-op edit (`editing && amount === originalAmountText`) reuse `editing.shares` verbatim; seed/validate/compute the split against `finalCents`, not the re-parsed display `cents`. Keep the existing parent-total guard. Make T003 pass. ✅ Fixed via render-scope `effectiveCents` + `reuseStoredShares`; 26 TxForm tests + full suite (942) green, `tsc` clean, vectors byte-identical.
- [X] T005 [P] [US1] Write FAILING repro test for the non-USD settle-up cent (B9-settle) in `web/test/web/settle-up-currency.test.tsx`: assert the current GBP settle-up transfer amount drifts ±1¢ from `balanceBetween`. ✅ Repro confirmed (wrote 12¢ for an 11¢ balance).
- [X] T006 [US1] Fix B9-settle: carry the exact USD-cents balance into the transfer prefill (reuse `amountCents` verbatim when the amount field is untouched) so the transfer zeroes the balance. Make T005 pass. ✅ Extended the `originalAmountText`/`effectiveCents` guard to cover `initialTransfer` (fix in `TxForm.tsx`, not BalanceSummary — the drift is at the prefill→field seam).
- [X] T007 [P] [US1] Write FAILING repro test for B7: mock the Supabase client so the primary write fails AND the compensating delete/update also fails; assert the current code drops/normalizes the share-less row. ✅ Added to `web/test/store.test.tsx` (reuses the sibling rollback harness) — repro confirmed (row dropped despite failed delete).
- [X] T008 [US1] Fix B7 in `web/lib/store.tsx` (`addTransaction`/`updateTransaction`): check the results of the compensating delete/update/`writeShares`; on failure keep the error banner up and do NOT present the affected row as consistent. Make T007 pass. ✅ Checked-compensation with a "reload to reconcile" flag; single-failure paths unchanged. (True atomicity via RPC stays out of scope.)
- [X] T009 [US1] Verify US1: `npm test` (T003/T005/T007 now pass, all suites green — 944), `npx tsc --noEmit` clean, `shared/test-vectors` byte-identical (no money-math output change). ✅
...
- [X] T022 [P] [US4] B3 (JS): wire `web/lib/scan/useScanFlow.ts` to `ScanPlugin.onPageCaptured` (accumulate pages, parse on an explicit "Done"); add a mocked-plugin test asserting multi-page pages are retained. ✅ `useScanFlow` subscribes to `onPageCaptured` before `capture()`, accumulates pages in a ref, and re-parses the growing document per page (the reducer's `capture/parsed` replaces the candidate list from the full set) so no page is dropped — pages arrive while the native camera is still up, so the re-parse never disrupts a review. `onPageCaptured` now returns a `PluginListenerHandle`; the listener is torn down on the next session start, on error, and on unmount. NOTE: the native contract has no explicit "done" event (flagged unsettled in the plugin contract) — the session ends when Done dismisses the camera (T023); incremental re-parse is the JS-only model that retains every page with today's contract. Added 2 tests (multi-page retained + listener cleanup); 970 green, tsc clean.
...
- [X] T031 [P] [US6] Write a FAILING render-count test in `web/test/store/context-render-isolation.test.tsx`: assert that an unrelated state change (one optimistic add / FX refresh) currently re-renders an unrelated `TransactionRow`. ✅ Deterministic counter (spy on the money formatter, keyed by each row's unique cents — immune to Profiler-bailout ambiguity); repro confirmed failing pre-fix (row re-renders 1→2 on an unrelated add) and passing post-fix.
- [X] T032 [US6] P4: split `web/lib/store.tsx` into a stable actions/services context (mutations + `resolveUser`/`t`/`formatMoney`/`ownersDisplay`, `useCallback`/`useMemo`-stabilized) vs a changing-data context, behind the SAME `useApp()` surface (no consumer import changes). `formatMoney` identity must still change on currency/rate/locale change. ✅ Two internal contexts (`DataCtx` + memoized `ServicesCtx = {rate,formatMoney,t,resolveUser,ownersDisplay}`); `useApp()` re-merges both (public surface unchanged, all ~50 consumers untouched); added `useAppServices()` for the rows. `formatMoney` memo deps `[currency, rate, locale]` (rate deps `[rates]`) so it still changes on FX/currency/locale. `AppData`/`AppServices` types enforce field completeness (tsc green).
- [X] T033 [US6] P4: `React.memo` `web/components/transactions/TransactionRow.tsx` and the desktop `TxRow` in `web/components/web/TransactionsDesktop.tsx`. Make T031 pass (unrelated mutation skips unchanged rows). ✅ Both rows use `useAppServices()` + `React.memo` with a custom comparator (`tx` identity + `selected` only; callbacks are pure fns of `tx` and stable setters, so ignoring their churn is safe). Desktop `selectRow` switched to a functional `setSelectedId` update so a skipped row's stale `onClick` closure can't toggle against a stale `selectedId`.
- [X] T034 [US6] P5: column-project the `select('*')` queries in `web/lib/store.tsx` `loadAll` (`transactions`, `transaction_shares`, `users`) to exactly the fields the app reads; store tests stay green with identical data. (No windowing — deferred.) ✅ Explicit column lists verified against the schema migrations (post-`20260616120000` shares = `transaction_id,person_id,amount_cents`; `paid_by` per `20260618120000`). Behaviorally byte-identical today (those tables have exactly the projected columns) while guarding future column bloat + pairing with FR-018. Note: the Supabase mock ignores the column list, so this is verified by schema/consumer review + CI, not unit tests.
- [X] T035 [US6] Verify US6: `npm test` (all store + component behavior identical), `npx tsc --noEmit`. ✅ 962 tests green (incl. the new T031 guard + all regression-vector parity suites byte-identical, no regen), `tsc --noEmit` clean, `npm run build` static export succeeds (11 routes prerendered). Ledger virtualization left as a deferred fast-follow (research D4 — no scroll/visual-regression-free path in scope).
...
- [X] T036 [US7] FR-018: type the Supabase→domain boundary — generate `web/lib/supabase/database.types.ts` (`supabase gen types`) and type the client `SupabaseClient<Database>`, OR (fallback) add typed `Row` interfaces + a mapper module; replace the `data as T[]` / `(m: any)`/`(l: any)` casts in `web/lib/store.tsx` `loadAll` with checked conversions, kept in lockstep with the T034 column lists. ✅ No `supabase gen types` in-sandbox → fallback: hand-written `web/lib/supabase/rows.ts` (schema-mirror `*Row` types); each loadAll read is asserted to its Row type and assigned to domain-typed state, so a renamed/removed column or changed enum fails to compile at the boundary. Removed all `data as T[]` and `(m/l: any)` casts. Proven: renaming a Row column fails `tsc` at `setUsers`.
- [X] T037 [P] [US7] FR-019: add `isTransfer(tx)` + `transferParties(tx)` in `web/lib/types.ts` (or new `web/lib/transaction.ts`); route the ~8 hand-branched sites (`store`, `TxForm`, `TransactionRow`, `TransactionDetailBody`, `TransactionsDesktop`, `TxModalWeb`, `balances`, `TransactionDetailModal`) through it. Grep confirms no direct `kind === 'transfer'`/`owner_ids[0]` outside the accessor.
- [X] T038 [P] [US7] FR-020: extract `web/lib/useMonthAccordion.ts` from the mobile+desktop duplication and use it in `web/app/(app)/transactions/page.tsx` and `web/components/web/TransactionsDesktop.tsx`.
- [X] T039 [P] [US7] FR-020: extract a shared `<TxFormBody>` component from the duplicated assembly in `web/components/web/TxModalWeb.tsx` and `web/components/web/TxForm.tsx`; both wrap it. ✅ Extracted `TxFormBody` (copy-from-recent affordance + fields + save-and-add-another) in `TxForm.tsx`, used by both the modal and the drawer. Narrow, pixel-identical (same JSX/order) — each surface keeps its own chrome + copy-picker wrapper (which legitimately differ), so the prior visual-change risk doesn't apply. 27 tx-form tests green.
- [X] T040 [P] [US7] FR-021: purge the ~200 dead i18n keys (diff catalog keys vs reachable `t()` calls + an allowlist of dynamic sources) across `web/lib/i18n/*`; add `web/test/i18n/catalog-reachability.test.ts` asserting every key is reachable or allowlisted. ✅ Removed 31 unreachable keys/catalog (155 entries): demo-mode/local-user/personal-vs-shared/sync-from-server leftovers. Reachability decided by TypeScript's scanner (comment/template-aware; regex breaks on apostrophes/backticks in comments) + cross-checked absent from source; no dynamic `t()` construction exists. Guard added + verified it fails on a reintroduced dead key. Allowlist empty (every dynamic label source is itself a source literal).
...
- [X] T042 [US7] Verify US7: `npx tsc --noEmit` (typed boundary — a scratch column rename now fails), `npm test` (guard + all green), and the grep checks from quickstart.md §US7. ✅ `tsc` clean; 968 tests green (incl. the reachability guard); FR-019 invariant logic centralized in `transferParties` (remaining `kind === 'transfer'` sites are display-label checks); `useMonthAccordion`/`TxFormBody` each have a single definition.

...
- **four billing edge functions** under `supabase/functions/` — Ortho's first server-side code (spec 018; §4.5);
...
├── functions/                   # spec 018 — the four billing edge functions (Deno; §4.5)
│   ├── _shared/billing/         # GENERATED byte-copy of services/billing/src — never edit by hand
│   ├── _shared/http.ts          # hand-written: CORS + the contract error envelope
